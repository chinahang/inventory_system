...
from utils.permissions import has_permission, page_context
...
def require_backup(request: Request, db: Session = Depends(get_db), code: str = "backup.view"):
    """按权限点校验；无权限时返回 None（保持原有 JSON 403 返回风格）。"""
...
def backup_page(request: Request, db: Session = Depends(get_db)):
    """Backup management page (needs backup.view)."""
    user = require_backup(request, db, "backup.view")
    if not user:
    ...
    return templates.TemplateResponse("backup.html", page_context(
        request, db, user,
        backups=backups,
        warn_retention=warn,
        max_backups=7
...
def start_backup(request: Request, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    """Start backup job."""
    user = require_backup(request, db, "backup.manage")
    if not user:
        return JSONResponse({"error": "权限不足"}, status_code=403)
